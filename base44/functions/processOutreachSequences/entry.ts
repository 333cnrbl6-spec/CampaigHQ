...
Deno.serve(async (req) => {
  try {
    ...
    for (let seqIdx = 0; seqIdx < matchingSequences.length; seqIdx++) {
     const sequence = matchingSequences[seqIdx];
     if (!contact) continue;
     ...
     for (let msgIdx = 0; msgIdx < sequence.messages.length; msgIdx++) {
       ...
       try {
         ...
         const logEntry = await base44.asServiceRole.entities.OutreachLog.create({
         ...
         console.error(`Failed to schedule message for sequence ${sequence.id}:`, err.message);
       }
     }

     // Update sequence sent count with error handling
     try {
       ...
       console.error(`Failed to update sequence count for ${sequence.id}:`, err.message);
     }

     // Rate limit: pause between sequences
     if (seqIdx + 1 < matchingSequences.length) {
       await delay(300);
     }
    }
