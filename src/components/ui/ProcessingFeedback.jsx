...
/**
 * ProcessingFeedback — reusable animated feedback panel for any long-running operation.
 * 
 * Props:
 *   label       string   — primary status headline
 *   detail      string   — supporting description
 *   tips        string[] — rotating tips shown every 8s (optional)
 *   step        number   — current step number (optional)
 *   totalSteps  number   — total steps (optional, shows progress bar)
 *   className   string
 */
    ...
    <div className={cn('bg-primary/5 border border-primary/20 rounded-xl p-5 space-y-3', className)}>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1 text-xs text-muted-foreground flex-shrink-0">
          ...
          {totalSteps > 1 && (
            <span className="ml-2 font-medium">{step}/{totalSteps}</span>
          )}
        </div>
      </div>
      ...
      {/* Indeterminate shimmer for single-step operations */}
      ...
      {tips.length > 0 && (
        <div className="text-xs text-muted-foreground bg-white/60 rounded-lg px-3 py-2 border border-primary/10 flex items-start gap-2">
          ...
          <span className="transition-opacity duration-500">{tips[tipIndex]}</span>
        </div>
      )}
    </div>
  );
